...
export default class View {
  constructor(template) {
    ...
    this.loginBtn = qs(".login_btn");
    this.logoutBtn = qs(".logout_btn");
  ...
  updateFooterButtons(route) {
    qs(".btn-filter.btn-filter-active", this.main).classList.remove(
      "btn-filter-active"
    );
    qs(`.btn-filter[href="#/${route}"]`, this.main).classList.add(
      "btn-filter-active"
    );
  }
  ...
  toggleAuthBtnVisibility(isUserLoggedIn) {
    //debugger;
    if (isUserLoggedIn) {
      if (!this.loginBtn.className.includes("hidden")) {
        this.loginBtn.classList.add("hidden");
      }
      this.logoutBtn.classList.remove("hidden");
    } else {
      if (!this.logoutBtn.className.includes("hidden")) {
