  ...
  itemsList(items) {
    return items.reduce((a, i) => {
      return (
        ...
        `
                <li class="${
                  i["is_completed"] ? "list_item list_item-done" : "list_item"
                }" id="${i.id}">
                    <div class="list_item_content flex align-center justify-center">
                        <input type="checkbox" class="toggle_item pointer" id="toggle${
                          i.id
                        }" ${i["is_completed"] ? "checked" : ""}>
                        <label for="toggle${
                          i.id
                        }" class="checkbox-toggle_item pointer"></label>
                        <span class="list_item_title" title="${escapeForHTMLAttributes(
                          i.content
                        )}">${escapeForHTML(i.content)}</span>
